...
const shouldBuyFidelity = (times) => {
  const TICKET_NORMAL = 12;
  const TICKET_FIDELITY = 250;
  const DISCOUNT = 0.75;
  const totalTicketNormal = 12 * times;

  let totalTicketFidelity = TICKET_FIDELITY;
  for (let i = 1; i <= times; i++) {
    totalTicketFidelity += TICKET_NORMAL * DISCOUNT ** i;
  }
  return totalTicketFidelity < totalTicketNormal;
};
